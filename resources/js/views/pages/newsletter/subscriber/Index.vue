...
<div>
  ...
  <div v-if="isFetched" class="is-loaded">
    ...
    <page-header>
      <h1>Subscribers</h1>
      <router-link :to="{ name: 'subscriber-create' }" class="feather-icon feather-icon--prepend">
    ...
    <div class="listing" v-if="subscribers.length">
      <div
        ...
        v-for="p in subscribers"
        :key="p.id"
      >
        <div class="listing__item-body">
          {{ p.firstname }} {{ p.name }} <separator /> {{ p.email }} <pill class="is-verified" v-if="p.verified_at">bestätigt</pill>
        </div>
        <list-actions 
          :id="p.id" 
          :record="p"
  ...
  methods: {
    ...
    destroy(id, event) {
      if (confirm("Bitte löschen bestätigen!")) {
        this.isLoading = true;
        this.axios.delete(`/api/subscriber/${id}`).then(response => {
          this.fetch();
