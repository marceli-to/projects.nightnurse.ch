...
<div v-if="isFetched">
  <content-header>
    <template #icon>
      <router-link :to="{ name: 'employee-register', params: {companyUuid: $route.params.companyUuid} }" class="btn-add">
        <plus-sm-icon class="h-5 w-5" aria-hidden="true" />
      </router-link>
    </template>
    <template #title>
      {{ translate('Mitarbeiter verwalten') }}
    </template>
  </content-header>
    ...
    <list-item v-for="d in data" :key="d.uuid">
      <div class="flex items-center">
        <div v-if="d.register_complete">
          ...
          <pill v-if="d.role_id == 3">{{ translate('Admin') }}</pill>
        </div>
        <div v-else>
          {{ d.email }}
  ...
  <content-footer>
    <router-link :to="{ name: 'projects'}" class="btn-primary">
  ...
  data() {
    return {
      ...
      routes: {
        ...
        destroy: '/api/employee',
        invite: '/api/employee/invite'
      },
