          ...
          <div 
            ...
            <span class="sm:flex sm:items-center justify-between text-xs font-mono pb-1 pt-4" v-if="(message.files.length > 3)">
              <a 
                ...
                :class="[message.private || message.internal ? 'text-gray-400' : 'text-slate-100', 'flex items-center no-underline hover:text-highlight mt-3 sm:mt-0']"
                v-if="hasTruncateFiles">
                <chevron-down-icon class="h-4 w-4" aria-hidden="true" />
                <span :class="[message.private || message.internal ? 'text-gray-400' : 'text-slate-100', 'hover:text-highlight inline-block ml-2']">{{ translate('Mehr anzeigen') }} ({{message.files.length - 3}})</span>
              </a>
              <a
                href="javascript:;" 
                @click="toggle(message.uuid)"
                ...
                <span :class="[message.private ? 'text-gray-400' : 'text-slate-100', 'hover:text-highlight inline-block ml-2']">{{ translate('Weniger anzeigen') }}</span>
              </a>
              ...
              <a 
                ...
                <span :class="[message.private ? 'text-gray-400' : 'text-slate-100', 'hover:text-highlight inline-block ml-2']">{{ translate('Download als ZIP') }}</span>
              </a>
